...
def reco():
    ...
    distance = []
    distance_file = 'others\distance'
    danger = []
    danger_file = 'others\danger'
            ...
            if len(classIds) != 0:
                ...
                if str(classNames[classIds[0]-1]) in danger:
                    if count == 1:
                        ...
                        print(str(classNames[classIds[0] - 1]))
                        count+=1

                if check.count(classIds[0]) <= 4:
                    speak(str(classNames[classIds[0] - 1])+" detected in '{}' meter".format(str(distance[classIds[0]-1])))
